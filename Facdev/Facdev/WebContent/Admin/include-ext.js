...
(function () {
    ...
    var scriptEls = document.getElementsByTagName('script'),
        ...
        extVersion = getQueryParam('version') || 'ext',
        ...
        i = 1,
        neptunePath,
        isDebug = getQueryParam('debug'),
        userCDN = getQueryParam('cdn') == '1' ? true: false;

    rtl = rtl && rtl.toString() === 'true'
    while (i--) {
        path = path.substring(0, path.lastIndexOf('/'));
    }
    path = path + '/'+extVersion;
    ...
    if (userCDN) {
        document.write('<link href="http://cdn.sencha.io/ext/gpl/4.2.0/resources/css/ext-all-neptune.css" rel="stylesheet" />');
        document.write('<script src="http://cdn.sencha.com/ext/gpl/4.2.0/ext-all.js"></script>');
    }
    else {
        if (includeCSS) {
            document.write('<link rel="stylesheet" type="text/css" href="' + path + '/resources/css/ext-all' + suffix + '-debug.css"/>');
        }
        document.write('<script type="text/javascript" src="' + path + '/ext-all' + (isDebug == 'true' ? '-debug' : '') + (rtl ? '-rtl' : '') + '.js"></script>');
    }
